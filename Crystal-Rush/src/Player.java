...
class Support {
	...
	public Coord estimate() {
		// Restituisce la coordinata (x,y) contenente minerali più vicina.
	...
	public Coord estimateTwo() {
		// Restituisce la coordinata (x,y) contenente minerali più vicina considerando
		// la posizione dei robot nemici.
		int col = -1;
		for (int j = 1; j < board.width; j++) {
			for (int i = 0; i < board.height; i++) {
				if (board.getCell(new Coord(j,i)).ore >= 1 && !board.myRadarPos.contains(new Coord(j,i))
		...
		if (col != -1) {
			Coord best = null;
			ArrayList<Entity> enemy = (ArrayList<Entity>) board.opponentTeam.robots;
			for (int i = 0; i < board.height; i++) {
				if (board.getCell(new Coord(col,i)).ore >= 1 && !board.myRadarPos.contains(new Coord(col,i))
						&& !board.myTrapPos.contains(new Coord(col,i))) {
					if (best == null) {
						best = new Coord(col,i);
					} else {
						Coord temp = new Coord(col,i);
						if (distance(best, enemy) > distance(temp, enemy))
							best = temp;
					}
				}
			}
	...
	public Coord estimateThree(int range) {
		// Restituisce la coordinata (x,y) contenente minerali più vicina (entro un
		// certo range) considerando la posizione dei robot nemici.
		int col = -1;
		for (int j = 1; j < board.width; j++) {
			for (int i = 0; i < board.height; i++) {
				if (board.getCell(new Coord(j,i)).ore >= 1 && !board.myRadarPos.contains(new Coord(j,i))
		...
		if (col != -1) {
			...
			for (int h = 0; h < range; h++) {
				for (int i = 0; i < board.height; i++) {
					if (board.getCell(new Coord(col + h,i)).ore >= 1
							&& !board.myRadarPos.contains(new Coord(col + h,i))
							&& !board.myTrapPos.contains(new Coord(col + h, i))) {
						if (best == null) {
							best = new Coord(col + h,i);
						} else {
							Coord temp = new Coord(col + h,i);
							if (distance(best, enemy) > distance(temp, enemy))
								best = temp;
						}
					}
	...
	public Coord estimateFour() {
		//// Restituisce la coordinata (x,y) contenente minerali più vicina 
		//  considerando lo storico della posizione dei robot nemici.
		int col = -1;
		for (int j = 1; j < board.width; j++) {
			for (int i = 0; i < board.height; i++) {
				if (board.getCell(new Coord(j,i)).ore >= 1 && !board.myRadarPos.contains(new Coord(j,i))
			...
			for (int i = 0; i < board.height; i++) {
				if (board.getCell(new Coord(col,i)).ore >= 1 && !board.myRadarPos.contains(new Coord(col,i))
					...
					if (best == null) {
						...
						Coord temp = new Coord(col , i);
						if (distance(best) > distance(temp))
							best = temp;
					}
				}
			}
	...
	private int distance(Coord c) {
		int totDistance = 0;
		for (int i = 0; i < forecastMatrix.length; i++) {
			for (int j = 0; j < forecastMatrix[i].length; j++) {
                 if(forecastMatrix[i][j]>0) totDistance=totDistance+(c.distance(new Coord(j,i))*forecastMatrix[i][j]);
			}
		}
		return totDistance;
	}

	private int distance(Coord c, ArrayList<Entity> enemy) {
		int totDistance = 0;
		for (int i = 0; i < enemy.size(); i++) {
			if (enemy.get(i).isAlive()) {
				totDistance = totDistance + c.distance(enemy.get(i).pos);
			}
		}
		return totDistance;
	}

	public void estimate(ArrayList<Entity> enemy) {
		for (int i = 0; i < enemy.size(); i++) {
			Coord c = enemy.get(i).pos;
			if (enemy.get(i).isAlive()) {
				forecastMatrix[c.y][c.x] = forecastMatrix[c.y][c.x] + 1;
				/*
				 * if (board.cellExist(new Coord(c.x + 1, c.y))) forecastMatrix[c.y][c.x + 1]++;
				 * if (board.cellExist(new Coord(c.x - 1, c.y))) forecastMatrix[c.y][c.x - 1]++;
				 * if (board.cellExist(new Coord(c.x, c.y + 1))) forecastMatrix[c.y + 1][c.x]++;
				 * if (board.cellExist(new Coord(c.x, c.y - 1))) forecastMatrix[c.y - 1][c.x]++;
				 * if (board.cellExist(new Coord(c.x - 1, c.y - 1))) forecastMatrix[c.y - 1][c.x
				 * - 1]++; if (board.cellExist(new Coord(c.x + 1, c.y - 1))) forecastMatrix[c.y
				 * - 1][c.x + 1]++; if (board.cellExist(new Coord(c.x + 1, c.y + 1)))
				 * forecastMatrix[c.y + 1][c.x + 1]++; if (board.cellExist(new Coord(c.x - 1,
				 * c.y + 1))) forecastMatrix[c.y + 1][c.x - 1]++;
				 */
			}
		}
		/*
		 * Coord best = null; int numVisit = 0; for (int i = 0; i <
		 * forecastMatrix.length; i++) { for (int j = 1; j < forecastMatrix[i].length;
		 * j++) { if (numVisit < forecastMatrix[i][j]) { numVisit =
		 * forecastMatrix[i][j]; forecastMatrix[i][j] = 0; best = new Coord(j, i); } } }
		 * return best;
		 */
	}
